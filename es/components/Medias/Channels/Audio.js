...
var Audio = function (_Component) {
    ...
    function Audio() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, _Component.call.apply(_Component, [this].concat(args))), _this), _this.isPlaying = function () {
            ...
            if (!isIE() && _this.audio.currentTime < _this.audio.duration - FLOAT_IMPRECISION) _this.props.dispatch({ type: 'AUDIO_IS_NOT_READY' });
        }, _this.handleCanPlayThrough = function () {
            if (!isIE()) _this.props.dispatch({ type: 'AUDIO_IS_READY' });
        }, _this.handleSeeking = function () {
            if (isIE()) _this.props.dispatch({ type: 'AUDIO_IS_NOT_READY' });
